...
class ResponseSincrono {
    constructor(statusEnvio, statusConsulta, statusDownload, cStat, motivo, xMotivo, nsNRec, chNFCom, nProt, xml, json, pdf, erros) {
...
async function emitirNFComSincrono(conteudo, tpAmb, tpDown, caminhoSalvar) {

    let respostaSincrona = new ResponseSincrono();

    let emissaoResponse = await emitir.sendPostRequest(conteudo)

    if ((emissaoResponse.status == 200) || (emissaoResponse.status == -6)){
    ...
    else if ((emissaoResponse.status == -999) || (emissaoResponse.status == -5 || (emissaoResponse.status == -7) || (emissaoResponse.status == -8))) {
        respostaSincrona.motivo = emissaoResponse.motivo
    }
...
module.exports = { ResponseSincrono, emitirNFComSincrono }
